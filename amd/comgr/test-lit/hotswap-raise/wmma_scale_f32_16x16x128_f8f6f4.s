...
; Per-row A-scale gather (buildScaleFactorVec + emitWMMAScaleF8F6F4toMFMA in
; wmma-lowering.cpp): each lane's 4 MFMA accumulator elements map to output
; rows GroupBase + 4*(lane/16) + g (g=0..3) sharing column lane%16, so the A
; (row) scale must be gathered from a distinct source lane per row -- not the
; lane's own column-indexed operand. Pin the address arithmetic and the four
; distinct per-row bpermutes of the SAME A-scale source.
